...
package com.greensock {
	...
	public class TweenMax extends TweenLite implements IEventDispatcher {
		...
		TweenPlugin.activate([// ACTIVATE (OR DEACTIVATE) PLUGINS HERE...
						...
						if (pt.name == prop) {
							if (pt.isPlugin) {
								...
								if (plugin == null) {
									...
									plugin.add(pt.target, prop, pt.start, pt.change);
									// using a single plugin for rounding speeds processing
								}
		...
		public function updateTo(vars : Object, resetDuration : Boolean = false) : void {
			...
			if (resetDuration && this.timeline != null && this.cachedStartTime < this.timeline.cachedTime) {
				...
				if (this.gc) {
					...
					// ensures that any necessary re-sequencing of TweenCores in the timeline occurs to make sure the rendering order is correct.
				}
			}
		...
		override public function renderTime(time : Number, suppressEvents : Boolean = false, force : Boolean = false) : void {
			...
			if (_repeat != 0) {
				...
				if (isComplete) {
					...
					// originally this.cachedTotalTime % cycleDuration but floating point errors caused problems, so I normalized it. (4 % 0.8 should be 0 but Flash reports it as 0.79999999!)
			...
			if (!this.active && !this.cachedPaused) {
				this.active = true;
				// so that if the user renders a tween (as opposed to the timeline rendering it), the timeline is forced to re-render and align it with the proper time/frame on the next rendering cycle. Maybe the tween already finished but the user manually re-renders it as halfway done.
			}
		...
		public static function set globalTimeScale(n : Number) : void {
			...
			if (TweenLite.rootTimeline == null) {
				TweenLite.to({}, 0, {});
				// forces initialization in case globalTimeScale is set before any tweens are created.
			}
