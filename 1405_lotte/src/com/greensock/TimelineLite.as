...
package com.greensock {
	...
	public class TimelineLite extends SimpleTimeline {
		...
		// Ensures that TweenLite instances don't overwrite each other before being put into the timeline/sequence.
		/** @private **/
		...
		override public function addChild(tween : TweenCore) : void {
			if (!tween.cachedOrphan && tween.timeline) {
				...
				// removes from existing timeline so that it can be properly added to this one. Even if the timeline is this, it still needs to be removed so that it can be added in the appropriate order (required for proper rendering)
			}
			tween.timeline = this;
			if (tween.gc) {
			...
			setDirtyCache(true);
			if (tween.cachedPauseTime || tween.cachedPauseTime == 0) {
				// faster than isNaN()
		...
		public function prependMultiple(tweens : Array, align : String = "normal", stagger : Number = 0, adjustLabels : Boolean = false) : Array {
			...
			// dump them into a new temporary timeline initially so that we can determine the overall duration.
		...
		override public function get duration() : Number {
			if (this.cacheIsDirty) {
				var d : Number = this.totalDuration;
