...
package com.greensock {
	...
	public class BlitMask extends Sprite {
		/** @private **/
		public static var version : Number = 0.22;
		 ...
		 */
		public function BlitMask(target : DisplayObject, x : Number = 0, y : Number = 0, width : Number = 100, height : Number = 100, smoothing : Boolean = false, autoUpdate : Boolean = false, fillColor : uint = 0x00000000) {
			super();
			_width = width;
			_height = height;
			_scaleX = _scaleY = 1;
		...
		protected function _render() : void {
			...
			if (x + _width < _bounds.x || y + _height < _bounds.y || x > _bounds.right || y > _bounds.bottom) {
				g.clear();
			...
			if (y <= _bounds.y) {
				_destPoint.y = _bounds.y - y;
				_sliceRect.y = 0;
			} else {
				_destPoint.y = 0;
				_sliceRect.y = (y - _bounds.y) - (row * _gridSize);
			}
			if (x <= _bounds.x) {
				_destPoint.x = _bounds.x - x;
				if (!_smoothing) {
					_destPoint.x += 1;
					// necessary compensation (not entirely sure why yet)
				}
				_sliceRect.x = 0;
			} else {
				_destPoint.x = 0;
				_sliceRect.x = (x - _bounds.x) - (column * _gridSize);
			}

			// offset the values to avoid slight vibrations during animation. Adjust for these in the Matrix that gets passed to beginBitmapFill()
			var xOffset : Number = 1 - (_sliceRect.x % 1);
			if (xOffset > 0.5) {
				xOffset -= 1;
			}
			_sliceRect.x += xOffset - 1;
			// subtract 1 to make sure the whole image gets included - without this, a very slight vibration can occur on the edge during animation.
			var yOffset : Number = 1 - (_sliceRect.y % 1);
			if (yOffset > 0.5) {
				yOffset -= 1;
			}
			_sliceRect.y += yOffset - 1;
			...
			if (_smoothing) {
				_drawRect.y = _destPoint.y;
				while (row <= maxRow) {
					bd = _grid[row][0];
					_drawRect.height = bd.height - _sliceRect.y;
					_drawRect.x = xDestReset;
					_sliceRect.x = xSliceReset;
					column = columnReset;
					while (column <= maxColumn) {
						bd = _grid[row][column];
						_drawRect.width = bd.width - _sliceRect.x;
						_tempMatrix.tx = _drawRect.x - _sliceRect.x;
						_tempMatrix.ty = _drawRect.y - _sliceRect.y;
						_bd.draw(bd, _tempMatrix, null, null, _drawRect, true);
						_drawRect.x += _drawRect.width - 1;
						_sliceRect.x = 0;
						column++;
					}
					_drawRect.y += _drawRect.height - 1;
					_sliceRect.y = 0;
					row++;
				}
			} else {
				_destPoint.y = (_destPoint.y + 0.5) >> 0;
				xDestReset = (xDestReset + 0.5) >> 0;
				xSliceReset = (xSliceReset + 0.5) >> 0;
				while (row <= maxRow) {
					bd = _grid[row][0];
					_sliceRect.height = (bd.height - _sliceRect.y + 0.5) >> 0;
			...
			_tempMatrix.tx = xOffset - 1;
			// subtract 1 to compensate for the pixel we added above where we created the offset values.
			_tempMatrix.ty = yOffset - 1;
		...
		public function set bitmapMode(value : Boolean) : void {
			if (_bitmapMode != value) {
				...
				if (_bitmapMode) {
					...
					// if cacheAsBitmap is true on both the _target and the BlitMask instance, the transparent areas of the mask will be...well...transparent which isn't what we want when bitmapMode is false (it could hide visible areas unless update(null, true) is called regularly, like if the target has animated children and bitmapMode is false)
					_target.mask = this;
				}
		...
		/** If <code>false</code> (the default), the bitmap (and the BlitMask's x/y coordinates) will be rendered only on whole pixels which is faster in terms of processing. However, for the best quality and smoothest animation, set <code>smoothing</code> to <code>true</code>. **/
		...
		/** The ARGB hexadecimal color that should fill the empty areas of the BlitMask. By default, it is transparent (0x00000000). If you wanted a red color, for example, it would be <code>0xFFFF0000</code>. **/
