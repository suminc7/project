...
package com.greensock.loading {
	...
	public class XMLLoader extends DataLoader {
		...
		public function XMLLoader(urlOrRequest : *, vars : Object = null) {
			...
			// just to make sure it wasn't overridden if the "format" special vars property was passed into in DataLoader's constructor.
		}
		...
		override protected function _dump(scrubLevel : int = 0, newStatus : int = 0, suppressEvents : Boolean = false) : void {
			...
			if (scrubLevel == 0) {
				_content = content;
				// super._dump() nulls "_content" but if the XML loaded and not the loading queue (yet), we should keep the XML content.
			}
		}
		...
		override protected function _calculateProgress() : void {
			...
			if (this.vars.integrateProgress == false) {
				// do nothing
			} else if (_loadingQueue != null && (uint(this.vars.estimatedBytes) < _cachedBytesLoaded || _loadingQueue.auditedSize)) {
				// make sure that estimatedBytes is prioritized until the _loadingQueue has audited its size successfully!
			...
			if (!_initted && _cachedBytesLoaded == _cachedBytesTotal) {
				...
				// don't allow the progress to hit 1 yet
			}
			_cacheIsDirty = false;
		}
		...
		protected static function _parseVars(xml : XML) : Object {
			...
			for each (var attribute:XML in list) {
				...
				if (s == "url") {
					...
					v.context = new LoaderContext(true, (value == "own") ? ApplicationDomain.currentDomain : (value == "separate") ? new ApplicationDomain() : new ApplicationDomain(ApplicationDomain.currentDomain), SecurityDomain.currentDomain);
					continue;
				}
