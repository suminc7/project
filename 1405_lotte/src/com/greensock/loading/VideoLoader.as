...
package com.greensock.loading {
	...
	public class VideoLoader extends LoaderItem {
		...
		public function VideoLoader(urlOrRequest : *, vars : Object = null) {
			...
			// just set it to a high number so that the progress starts out low.
		...
		override protected function _load() : void {
			...
			if (_videoPaused) {
				...
				// temporarily silence the audio because in some cases, the Flash Player will begin playing it for a brief second right before the buffer is full (we can't pause until then)
			} else {
				this.volume = _volume;
				// ensures the volume is back to normal in case it had been temporarily silenced while buffering
			}
		...
		override protected function _dump(scrubLevel : int = 0, newStatus : int = 0, suppressEvents : Boolean = false) : void {
			...
			if (scrubLevel >= 2) {
				...
				(_sprite as Object).gcProtect = (scrubLevel == 3) ? null : _ns;
				// we need to reference the NetStream in the ContentDisplay before forcing garbage collection, otherwise gc kills the NetStream even if it's attached to the Video and is playing on the stage!
				_ns.client = {};
				_video = null;
				_ns = null;
				_nc = null;
		...
		public function gotoVideoTime(time : Number, forcePlay : Boolean = false, skipCuePoints : Boolean = true) : void {
			...
			if (_initted && _bufferedOnce && changed) {
				// don't seek() until metaData has been received, otherwise it can prevent it from ever being received.
				_ns.seek(time);
				_bufferFull = false;
			}
		...
		protected function _onBufferFull() : void {
			...
			if (_pauseOnBufferFull) {
				if (!_initted && getTimer() - _time < 10000) {
					_video.attachNetStream(null);
					// in some rare circumstances, the NetStream will finish buffering even before the metaData has been received. If we pause() the NetStream before the metaData arrives, it will prevent the metaData from ever arriving (bug in Flash) even after you resume(). So in this case, we allow the NetStream to continue playing so that metaData can be received, but we detach it from the Video object so that the user doesn't see the video playing. The volume is also muted, so to the user things look paused even though the NetStream is continuing to play/load. We'll re-attach the NetStream to the Video after either the metaData arrives or 10 seconds elapse.
					return;
				} else {
					_pauseOnBufferFull = false;
					this.volume = _volume;
					// Just resets the volume to where it should be because we temporarily made it silent during the buffer.
					_ns.seek(_forceTime || 0);
					_video.attachNetStream(_ns);
					// in case it was removed
					_ns.pause();
					// don't just do this.videoPaused = true because sometimes Flash fires NetStream.Play.Start BEFORE the buffer is full, and we must check inside the videoPaused setter to see if if the buffer is full and wait to pause until it is.
				}
			}
			if (!_bufferFull) {
				_bufferFull = true;
		...
		protected function _forceInit() : void {
			...
			if (_pauseOnBufferFull) {
				_video.attachNetStream(_ns);
				// if the NetStream isn't attached, then the Video object doesn't get resized properly in the ContentDisplay object.
				(_sprite as Object).rawContent = _video;
				// resizes it appropriately
				_video.attachNetStream(null);
			} else {
				(_sprite as Object).rawContent = _video;
				// resizes it appropriately
		...
		protected function _metaDataHandler(info : Object) : void {
			...
			if ("width" in info) {
				...
				_video.scaleY = info.height / 160;
				// MUST use 160 as the base width and adjust the scale because of the way Flash reports width/height/scaleX/scaleY on Video objects (it can cause problems when using the scrollRect otherwise)
			}
			_forceInit();
		...
		protected function _statusHandler(event : NetStatusEvent) : void {
			...
			if (code == "NetStream.Play.Start") {
				// remember, NetStream.Play.Start can be received BEFORE the buffer is full.
				if (!_pauseOnBufferFull) {
			...
			if (code == "NetStream.Play.Stop") {
				...
				if (_videoPaused) {
					return;
					// Can happen when we seek() to a time in the video between the last keyframe and the end of the video file - NetStream.Play.Stop gets received even though the NetStream was paused.
				}
				...
				if (this.autoAdjustBuffer && loadRemaining > videoRemaining) {
					_ns.bufferTime = videoRemaining * (1 - (videoRemaining / loadRemaining)) * 0.9;
					// 90% of the estimated time because typically you'd want the video to start playing again sooner and the 10% might be made up while it's playing anyway.
				}
		...
		protected function _enterFrameHandler(event : Event) : void {
			...
			if (_cachedBytesLoaded == _cachedBytesTotal && _ns.bytesTotal > 5 && (_initted || getTimer() - _time >= 10000)) {
				// make sure the metaData has been received because if the NetStream file is cached locally sometimes the bytesLoaded == bytesTotal BEFORE the metaData arrives. Or timeout after 10 seconds.
				_sprite.removeEventListener(Event.ENTER_FRAME, _enterFrameHandler);
				if (!_bufferedOnce) {
					_onBufferFull();
				}
		...
		public function set videoPaused(value : Boolean) : void {
			...
			if (_videoPaused) {
				...
				if (!_bufferedOnce) {
					...
					_sound.volume = 0;
					// temporarily make it silent while buffering.
					_ns.soundTransform = _sound;
				} else {
					_pauseOnBufferFull = false;
					this.volume = _volume;
					// Just resets the volume to where it should be in case we temporarily made it silent during the buffer.
					_ns.pause();
				}
				if (changed) {
					_sprite.removeEventListener(Event.ENTER_FRAME, _playProgressHandler);
				...
				if (_pauseOnBufferFull || !_bufferFull) {
					...
					_video.attachNetStream(_ns);
					// in case we had to detach it while buffering and waiting for the metaData
					_pauseOnBufferFull = false;
				}
				this.volume = _volume;
				// Just resets the volume to where it should be in case we temporarily made it silent during the buffer.
		...
		public function get videoTime() : Number {
			if (_videoComplete) {
				...
				// sometimes the NetStream reports a time that's greater than the duration so we must correct for that.
			} else {
				return _ns.time;
			}
		}
