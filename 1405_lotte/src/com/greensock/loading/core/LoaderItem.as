		...
		override public function auditSize() : void {
			if (_auditStream == null) {
				...
				if (!_isLocal || _url.substr(0, 4) == "http") {
					request.data = new URLVariables("cacheBusterID=" + (_cacheID++) + "&purpose=audit");
					// to work around bugs in Chrome and Firefox that can cause them to use the partially-loaded audit file in the cache instead of loading the full file normally.
				}
