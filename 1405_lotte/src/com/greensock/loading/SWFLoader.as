	...
	public class SWFLoader extends DisplayObjectLoader {
		...
		override protected function _dump(scrubLevel : int = 0, newStatus : int = 0, suppressEvents : Boolean = false) : void {
			...
			if (scrubLevel >= 1) {
				...
				super._dump(scrubLevel, newStatus, suppressEvents);
				_content = content;
				// super._dump() will null "_content", but if the swf has loaded but not the _queue, we should keep the content so that if resume() is called, it just starts loading the queue.
			}
		}
		...
		override protected function _calculateProgress() : void {
			...
			if (this.vars.integrateProgress == false) {
				// do nothing
			} else if (_queue != null && (uint(this.vars.estimatedBytes) < _cachedBytesLoaded || _queue.auditedSize)) {
				// make sure that estimatedBytes is prioritized until the _queue has audited its size successfully!
			...
			if (!_scriptAccessDenied) {
				...
				if (_content != null) {
					if (this.vars.autoPlay == false && _content is MovieClip) {
						...
						st.volume = 0;
						// just make sure you can't hear any sounds as it's loading in the background.
		...
		override protected function _completeHandler(event : Event = null) : void {
			...
			if (this.progress == 1) {
				...
				// now do the BitmapData.draw() test.
