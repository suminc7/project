	...
	public class LoaderMax extends LoaderCore {
		...
		override protected function _dump(scrubLevel : int = 0, newStatus : int = 0, suppressEvents : Boolean = false) : void {
			if (newStatus == LoaderStatus.DISPOSED) {
				...
				// must set it first so that when events from children are dispatched, it doesn't trigger other unnecessary actions.
		...
		override protected function _progressHandler(event : Event) : void {
			if (_dispatchProgress) {
				...
				if ((_cachedBytesLoaded != _cachedBytesTotal || _status != LoaderStatus.LOADING) && (bl != _cachedBytesLoaded || bt != _cachedBytesTotal)) {
					// note: added _status != LoaderStatus.LOADING because it's possible for all the children to load independently (without the LoaderMax actively loading), so in those cases, the progress would never reach 1 since LoaderMax's _completeHandler() won't be called to dispatch the final PROGRESS event.
		...
		override public function get status() : int {
			...
			if (_status == LoaderStatus.COMPLETED) {
				var statusCounts : Array = [0, 0, 0, 0, 0, 0];
				// store the counts of each type of status (index 0 is for READY, 1 is LOADING, 2 is COMPLETE, etc.
				var i : int = _loaders.length;
				while (--i > -1) {
