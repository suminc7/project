...
package com.framework.utils {
	...
	public class SWFWheel {
		...
		private static function triggerMouseEvent(delta : Number, ctrlKey : Boolean, altKey : Boolean, shiftKey : Boolean) : void {
			...
			if (!target) return;
			// create event.
			var event : MouseEvent = new MouseEvent(MouseEvent.MOUSE_WHEEL, true, false, target.mouseX, target.mouseY, null,                   // related object is only over and out. 
			ctrlKey, altKey, shiftKey, false,                  // dirty... 
			int(delta));
			// trigger event.
